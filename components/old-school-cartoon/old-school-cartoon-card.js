...
export default function OldSchoolCartoonCard(item) {
    return (
        <>
            <Link href={`/cartoon/${item.id}`}>
                <img
                    src={item.image}
                    className="rounded-3xl w-[210px] h-[320px] shadow-[0px_3px_8px_rgba(0,0,0,0.24)]"
                />
                <div className="mt-3">
                    <p className="font-medium text-[#0B3954] text-lg">{item.ct_title}</p>
                    <div className="flex items-center">
                        ...
                        <span className="ms-1 text-[#087E8B] font-medium">{item.view_count} times | {item.published_year.split("-")[0]}</span>
                    </div>
                </div>
            </Link>
        </>
